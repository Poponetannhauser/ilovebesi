...
import { PEKERJAAN_COLORS, BERAT_PER_METER, PANJANG_STANDAR_BATANG } from '../types';
import { getRekapDiameter, formatNumber, getTotalBerat } from '../utils/storage';
...
export const TabRekapitulasi: React.FC<Props> = ({ items }) => {
  ...
  // Rekap per pekerjaan
  const pekerjaanMap = new Map<TypePekerjaan, { totalBerat: number; totalPanjang: number }>();
  items.forEach(item => {
    const existing = pekerjaanMap.get(item.typePekerjaan) || { totalBerat: 0, totalPanjang: 0 };
  ...
  const rekapPekerjaan: RekapPekerjaanRow[] = Array.from(pekerjaanMap.entries()).map(([tp, d]) => ({
    typePekerjaan: tp,
  ...
  const colsDiameter: ColumnsType<RekapDiameterRow> = [
    {
      ...
      dataIndex: 'diameter',
      key: 'diameter',
      render: (val: DiameterBesi) => <span className="font-bold text-orange-400 font-mono text-sm">D{val}</span>,
    },
    {
      title: 'Berat/m (kg/m)',
      ...
      key: 'beratPerMeter',
      align: 'right',
      render: (val: number) => <span className="font-mono text-slate-500">{val}</span>,
    },
    {
      ...
      render: (val: number) => <span className="font-mono text-sky-600">{formatNumber(val, 2)}</span>,
    },
    {
      ...
      render: (val: number) => <span className="font-mono font-semibold text-green-400">{formatNumber(val, 2)}</span>,
    },
    {
      ...
      key: 'totalBeratTon',
      align: 'right',
      render: (val: number) => <span className="font-mono font-bold text-green-300">{val.toFixed(3)}</span>,
    },
    {
      ...
      render: (val: number) => <span className="font-mono font-bold text-purple-400">{val}</span>,
    },
  ];
  ...
  const colsPekerjaan: ColumnsType<RekapPekerjaanRow> = [
    {
      ...
      key: 'typePekerjaan',
      render: (val: TypePekerjaan) => <Tag color={PEKERJAAN_COLORS[val]} style={{ fontWeight: 600 }}>{val}</Tag>,
    },
    {
      title: 'Total Panjang (m)',
      dataIndex: 'totalPanjang',
    ...
    },
    {
      ...
      render: (val: number) => (
        <div className="flex items-center gap-2 justify-end">
          ...
          <span className="font-mono text-slate-300 text-xs w-12 text-right">{val.toFixed(1)}%</span>
        </div>
      ),
    },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Total Item', value: items.length.toString(), color: 'text-blue-400', unit: 'item' },
          { label: 'Total Panjang', value: formatNumber(totalPanjang, 2), color: 'text-blue-300', unit: 'm' },
          { label: 'Total Berat', value: formatNumber(totalBerat, 2), color: 'text-green-400', unit: 'kg' },
          { label: 'Total Berat', value: (totalBerat / 1000).toFixed(3), color: 'text-green-300', unit: 'ton' },
        ].map((card, i) => (
          <div key={i} className="glass-card p-4 text-center animate-fade-in-up" style={{ animationDelay: `${i * 0.05}s` }}>
            <p className="text-xs text-slate-500 uppercase tracking-wider font-semibold">{card.label}</p>
            <p className={`text-2xl font-bold mt-2 font-mono ${card.color}`}>{card.value}</p>
      ...
      <div className="glass-card p-4">
        ...
        <Table
          ...
          summary={() => (
            <Table.Summary>
              <Table.Summary.Row>
                <Table.Summary.Cell index={0}><span className="font-bold text-slate-800">TOTAL</span></Table.Summary.Cell>
                <Table.Summary.Cell index={1} />
                <Table.Summary.Cell index={2} align="right">
                  <span className="font-mono font-bold text-blue-300">{formatNumber(totalPanjang, 2)}</span>
                </Table.Summary.Cell>
                <Table.Summary.Cell index={3} align="right">
                  <span className="font-mono font-bold text-green-400">{formatNumber(totalBerat, 2)}</span>
                </Table.Summary.Cell>
                <Table.Summary.Cell index={4} align="right">
                  <span className="font-mono font-bold text-green-300">{(totalBerat / 1000).toFixed(3)}</span>
                </Table.Summary.Cell>
